...
public class SysCategoryDaoImpl implements SysCategoryDao {
	...
	public SysCategory getById(int id) {
		...
		DBPool dbc=new DBPool();
		try {
			String sql="select rownum num,t.* from(select * from sys_category where id=?) t where rownum<= 1";
	...
	public SysCategory getByName(String scgName) {
		...
		DBPool dbc=new DBPool();
		try {
			String sql="select rownum num,t.* from(select * from sys_category where category_name=? and is_delete=0) t where rownum<= 1";
	...
	public List<SysCategory> getSysCategoryByPage(int curPage, int size) {		
		...
		DBPool dbc=new DBPool();
		try {
			String sql="select * from(select rownum num,t.* from(select * from sys_category where is_delete=0 order by id DESC) t where rownum<= ?) where num>= ?";
