...
public class CategoryDaoImpl implements CategoryDao {
	...
	public Category getById(int id) {
		...
		DBPool dbc=new DBPool();
		try {
			String sql="select rownum num,t.* from(select * from category where id=?) t where rownum<= 1";
	...
	public Category getByName(String cgName, int userId) {
		...
		DBPool dbc=new DBPool();
		try {
			String sql="select rownum num,t.* from(select * from category where category_name=? and user_id=? and is_delete=0) t where rownum<= 1";
	...
	public List<Category> getCategoryByPage(int curPage, int size) {
		...
		DBPool dbc=new DBPool();
		try {
			String sql="select * from(select rownum num,t.* from(select * from category where is_delete=0 order by id DESC) t where rownum<= ?) where num>= ?";
			cgList = new ArrayList<Category>();
	...
	public List<Category> getCategoryByPageUserId(int curPage, int size, int userId) {
		...
		DBPool dbc=new DBPool();
		try {
			String sql="select * from(select rownum num,t.* from(select * from category where is_delete=0 and user_id=? order by id DESC) t where rownum<= ?) where num>= ?";
			cgList = new ArrayList<Category>();
