...
public class ArticleServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(action.equals("add")){//新建文章
			int userId = Integer.parseInt(request.getParameter("userId"));
			List<Category> cgList = cgService.getByUserId(userId);//根据userId查找用户的个人分类
			List<SysCategory> scgList = scService.getAllSysCategory();//获取所有系统分类			
		...
		}else if(action.equals("save")){//保存文章
			int userId = Integer.parseInt(request.getParameter("userId"));
		...
		}else if(action.equals("update")){//修改文章			
			int artId = Integer.parseInt(request.getParameter("artId"));			
			Article art = artService.getById(artId);
			List<SysCategory> scgList = scService.getAllSysCategory();//系统分类信息		
			request.setAttribute("scgList", scgList);
			request.setAttribute("art", art);
			request.getRequestDispatcher("UpdateArtical.jsp").forward(request, response);
		}else if(action.equals("manage")){//博文管理			 
			int userId = Integer.parseInt(request.getParameter("userId"));
			//添加用于分页
			...
			response.sendRedirect("article?action=manage&userId="+userId);
		}else if(action.equals("saveupdate")){//保存修改文章			
			int artId = Integer.parseInt(request.getParameter("artId"));
