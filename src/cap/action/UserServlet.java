...
public class UserServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(action.equals("login")){	//登陆		
		...
		}else if(action.equals("index")){//首页
			//跳转到首页			
			List<SysCategory> scList = scService.getAllSysCategory();//获取系统分类列表
			List<User> uList = artService.getActiveUser(2);	//获取 2 个活跃人数
			List<Article> tenList=artService.topTenArticle();//博文top10
			//分页
			String curPageStr = request.getParameter("curPage");//当前页码	
			PageControl pc=artService.getData(curPageStr);//当前页码文章信息
		...
		}else if(action.equals("myblog")){//获取作者所有博文
			int userId = Integer.parseInt(request.getParameter("userId"));
			BlogInfo bi = biService.getByuserId(userId);//个人博客信息
			...
			if (null == bi) {
				...
				PageControl pc=artService.getByPageUserId(curPageStr, userId);//获取作者当前页码博文
		...
		}else if(action.equals("myblogFilter")){//获取作者所有博文
			int userId = Integer.parseInt(request.getParameter("userId"));
			...
			if (null == bi) {
				...
				String curPageStr = request.getParameter("curPage");
				PageControl pc=artService.getByPageUserCategoryId(curPageStr, userId,categoryId);//获取作者当前页码博文
				request.setAttribute("userId", userId);
				request.setAttribute("blogName", blogName);
				request.setAttribute("blogAnnoucement", blogPropagate);
				request.setAttribute("artList", pc.getList());
		...
		}else if(action.equals("apply")){//初次注册开通博客
			int userId = Integer.parseInt(request.getParameter("userId"));
			...
			if (null != bi) {
				...
				res = biService.insertBlogInfo(userId, blogName, description, annoucement);//开通博客
			}
		...
		}else if(action.equals("profile")){//编辑个人信息
			//转到个人主页
			int id = Integer.parseInt(request.getParameter("id"));			
			User u = userService.getUserById(id);//根据id获取用户资料			
			Profile pf = profileService.getByuserId(id);//根据id获取个人信息		
			if (null != pf) {
				request.setAttribute("profile", pf);
			}			
			request.getSession().setAttribute("user", u);
			request.getRequestDispatcher("Profile.jsp").forward(request, response);
		}else if(action.equals("logout")){//登出
			...
			response.sendRedirect("user?action=index");
		}else if(action.equals("myblogindex")){//我的博客
			int userId = Integer.parseInt(request.getParameter("userId"));
			BlogInfo bi = biService.getByuserId(userId);
			List<Category> cgList = categoryService.getByUserId(userId);    //获取用户的个人分类			
		...
		}else if(action.equals("updateprofile")){//更新个人信息			
			...
			if (null != pf) {
				res = profileService.updateProfile(userId, firstName, lastName, gender, telephone);//如果有个人信息，则更新
				pf = profileService.getByuserId(userId);				
			} else {
				res = profileService.insertProfile(userId, firstName, lastName, gender, telephone);//如果无个人信息，则新建
			}
				
			if (res > 0) {
				resOfpf = userService.setIsProfile(userId);//设置个人信息完善				
			...
			response.sendRedirect("user?action=profile&id="+userId);
		}else if(action.equals("bloginfo")){//编辑博客信息
			int userId = Integer.parseInt(request.getParameter("userId"));
			BlogInfo bi = biService.getByuserId(userId);//根据userId获取博客信息		
			request.setAttribute("bi", bi);
			request.getRequestDispatcher("BlogInfo.jsp").forward(request, response);
		}else if(action.equals("updatebloginfo")){//更新个人博客信息
			response.setContentType("text/html; charset=UTF-8");
			request.setCharacterEncoding("UTF-8");
			int userId = Integer.parseInt(request.getParameter("userId"));
			String blogName = request.getParameter("blog_name");
			...
			int res = biService.updateBlogInfo(userId, blogName, description, propagate);//更新个人博客信息
